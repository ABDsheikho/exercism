...
aggregateScorers playerNames =
    aggregateToDict playerNames Dict.empty

        
aggregateToDict : List PlayerName -> Dict PlayerName Int -> Dict PlayerName Int
aggregateToDict playerNames playerGoalCounts =
    case playerNames of
        [] ->
            playerGoalCounts
        head::reast ->
            updateGoalCountForPlayer head playerGoalCounts
            |> aggregateToDict reast 
...
formatPlayer playerName playerGoalCounts =
    case Dict.get playerName playerGoalCounts of
        Nothing ->
            playerName ++ ": 0"
        Just goals ->
            representNameAndGoals ": " playerName goals
